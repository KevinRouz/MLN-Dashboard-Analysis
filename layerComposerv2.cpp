...
int main()
{
    ...
    FILE *f = fopen("layerComposer.conf", "r");
    char info[500];
...
// start timer: 
	...
	{
		sprintf(info1, "%s_%s_%s.net", layerID[0], compositionType, layerID[1]);
		f = fopen(info1, "w");
...
//					cout<<graph1->array[i].repVertex<<"\n";
		...
		sprintf(info, "sed -i '1s/^/%s_%s_%s\\n%d\\n%d\\n/' %s_%s_%s.net", layerID[0], compositionType, layerID[1], graph1->V, numberOfEdges[2], layerID[0], compositionType, layerID[1]);
		system(info);
...
//					cout<<graph1->array[i].repVertex<<"\n";
	...
	else if (strcmp(compositionType, "NOT") == 0)
	{
		...
		sprintf(info1, "%s[%s].net", compositionType, layerID[0]);
...
//		cout<<layer1List->dest<<"\n";
		...
		fclose(f);
// May 28, 2018: Edited the last parameter of the NOT List	
		sprintf(info, "sed -i '1s/^/%s[%s]\\n%d\\n%d\\n/' %s'['%s']'.net", compositionType, layerID[0], graph1->V, numberOfEdges[2], compositionType, layerID[0]);
		system(info);
